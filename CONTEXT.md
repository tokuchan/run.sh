...
### Command
A directory in the command tree that represents one invocable operation. A command directory may contain: `main.<ext>` (the program to run), `mount` or `mount.txt` (bind-mount configuration), `env` or `env.txt` (environment variables), `conf` or `conf.txt` (per-command settings), `help.md` or `help.txt` (user-facing documentation), and `fs/` (host-side mount sources). The command is the atomic unit of configuration — replaces Stem.

### Mount file
`commands/<cmd>/mount` (or `mount.txt`) — the per-command bind-mount list. Renamed from `run`/`run.txt` (ADR-0018) to read unambiguously as "the file that holds mounts," distinct from the project-wide `commands/run.conf`. Format is unchanged: one `/container/path` (optionally `:ro`) per line, `#` comments and blank lines ignored. Strictly mount specs — no key=value settings live here; those belong in [[Command conf]]. Loaded root→leaf same as `env`/`conf`; mount specs accumulate, destination conflicts resolve last-wins (deepest directory wins). A line that isn't a mount spec, comment, or blank is a hard error (exit 125) directing the user to `conf` instead of a silent drop or soft warning — this file is mounts and only mounts. If a leftover `run` or `run.txt` is found in any directory along the matched command path, run.sh exits 125 rather than silently failing to apply its mounts.
...
### Command load order
The sequence in which command configuration is applied: command tree root (`commands/mount`, `commands/env`) → each directory level along the command path in order → deepest matched command directory last. Mount specs accumulate across all levels; env var conflicts and mount destination conflicts are resolved last-wins (deepest directory wins).

### Command conf
`commands/<cmd>/conf` (or `conf.txt`) — a per-command settings file, `key = value` per line, `#` comments and blank lines ignored. Loaded by `load_command_config` alongside `mount` and `env` files, same root→leaf walk, same last-wins override semantics. Currently recognizes one key: `dispatch`. Unknown keys and unknown values are a hard error (exit 125), matching `commands/run.conf`'s strictness. The dividing line from [[Mount file]] is strict: `mount` holds only bind-mount specs, `conf` holds only key=value settings — neither file accepts the other's shape. See ADR-0017, ADR-0018.
...
### Runner metadata
Five environment variables injected by the runner into every command invocation — both container- and host-dispatched — regardless of what `commands/env` files specify. Read-only — cannot be overridden by command configuration. `RUN_PROJECT`: the runner's `basename $0` (e.g. `foo`). `RUN_COMMAND`: the slash-separated resolved command path (e.g. `release/build`). `RUN_ROOT`: the absolute host path of the run root, available inside the container via the project root mirror. `RUN_CONTAINER_RUNTIME`: the resolved container runtime binary (`podman` or `docker`), empty if neither is installed. `RUN_CONTAINER_IMAGE`: the configured image name, empty if `image` isn't set in `run.conf`. Deliberately named apart from the `RUN_RUNTIME`/`RUN_IMAGE` override inputs (see [[Container runtime]]) so a nested run.sh-based command never mistakes this invocation's resolved values for its own configuration — the same reasoning that keeps `RUN_ROOT` (output) distinct from `RUN_PROJECT_ROOT` (input). Scripts use `RUN_ROOT`/`RUN_PROJECT`/`RUN_COMMAND` to construct paths without hardcoding, and `RUN_CONTAINER_RUNTIME`/`RUN_CONTAINER_IMAGE` to invoke the container manager themselves (see [[Metadata-driven runtime access]]) — most relevant for [[Host dispatch]], where a command may need to drive a sibling container directly. See ADR-0019.

### Metadata-driven runtime access
The pattern of a command using the injected `RUN_CONTAINER_RUNTIME`/`RUN_CONTAINER_IMAGE` metadata instead of re-detecting podman vs. docker or hardcoding an image name — e.g. `"$RUN_CONTAINER_RUNTIME" run "$RUN_CONTAINER_IMAGE" ...`. Resolution is best-effort and non-fatal at injection time: a host-dispatched command with no need for a container runtime keeps working even when neither podman nor docker is installed, since `detect_runtime` only becomes a hard failure at the call sites that actually require a runtime (container dispatch, `--clean`, package management). A command that does need it and finds `RUN_CONTAINER_RUNTIME` empty fails on its own when it tries to invoke it. See ADR-0019.
...
### Command help
Per-command documentation stored as `help.md` or `help.txt` in the command directory. The first paragraph is the short description — used in auto-generated listings. `--help` after any command path prefix displays the help file for the deepest matched command directory (run.sh intercepts `--help` and never forwards it to `main.<ext>`). `--help` with no command path is intercepted at the very top of `main()`, before project-root detection runs, and shows only the top-level runner manual — never a command listing, since no run root has been resolved yet at that point.

### Command listing
The auto-generated output shown when the runner is invoked with a command path that has no `main.<ext>`. If a `help.md` or `help.txt` exists in the matched directory, its first paragraph is displayed before the listing. Displays only the immediate children of that directory (non-recursive), with the first sentence of each child's `help.md`/`help.txt` as a trailing description. `--help` produces a full recursive listing from the matched directory. `--list-commands` produces a machine-readable list of all commands in the entire tree: one per line, slash-separated path, tab-separated from its usage line (first sentence of `help.md`/`help.txt`); description omitted if no help file exists. The bare no-args invocation is a distinct case — see No-args cheat sheet.

### No-args cheat sheet
Invoking the runner with zero arguments always prints the short `usage()` cheat sheet and exits 0 — an unconditional invariant (ADR-0010) that holds regardless of project state, and never has side effects. Since ADR-0020, the cheat sheet is followed by a best-effort command listing: run root detection is attempted, and if it succeeds, the listing for the top-level `commands/` directory is appended below the cheat sheet. If no run root is found (e.g. a freshly-copied, not-yet-`--init`'d script), the cheat sheet is shown alone — this never becomes an error, unlike every other code path that depends on a run root. Distinct from `--help` (full manual, no listing) and `--list-commands` (machine-readable, recursive, no cheat sheet).
